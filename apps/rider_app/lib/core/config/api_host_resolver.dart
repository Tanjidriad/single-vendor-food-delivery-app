...
class ApiHostResolver {
  ...
  static Future<void> init(SharedPreferences prefs) async {
    ...
    _logger.i('API Host Candidates: $candidates');
    for (final base in candidates) {
      final resolved = await _tryResolve(base);
      if (resolved != null) {
        apiBaseUrl = resolved.apiBaseUrl;
        socketBaseUrl = resolved.socketBaseUrl;
        _logger.i('Successfully resolved API Base URL: $apiBaseUrl');
    ...
    _logger.w('All resolution attempts failed. Falling back to default.');
  ...
  static Future<({String apiBaseUrl, String socketBaseUrl})?> _tryResolve(
    ...
    try {
      _logger.i('Testing health endpoint at: $apiBaseUrl/health');
      final health = await dio.get<Map<String, dynamic>>('$apiBaseUrl/health');
      if (health.data?['status'] == 'ok') {
        _logger.i('Health check passed for $apiBaseUrl');
        if (kDebugMode) {
          try {
      ...
      _logger.w('Failed to connect to $apiBaseUrl: $e');
    }
